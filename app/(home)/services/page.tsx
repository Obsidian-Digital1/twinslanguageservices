		...
		<Main>
			<section className="relative overflow-hidden bg-twin-primary-main px-5 pt-36 pb-24 text-white md:px-10 md:pt-44 md:pb-32 lg:px-[8%]">
				<div className="mx-auto w-full max-w-350">
					...
					<h1 className="mt-7 max-w-275 text-balance text-[clamp(3.25rem,8vw,7rem)] leading-[0.95] font-bold tracking-[-0.05em]">
					...
					<p className="mt-8 max-w-190 text-pretty text-lg/relaxed text-white/75 md:text-xl">
						Choose the service that best matches your request. If you are unsure, contact us
						and we’ll help identify the right starting point.
					</p>
				</div>
			</section>

			<section className="bg-white px-5 py-20 md:px-10 md:py-28 lg:px-[8%]">
				<div className="mx-auto w-full max-w-350 divide-y divide-twin-primary-main/14 border-y border-twin-primary-main/14">
					{siteConfig.services.map((service, index) => (
						<article
							key={service.id}
							id={service.id}
							className="grid scroll-mt-28 gap-6 py-10 md:grid-cols-[80px_0.8fr_1.2fr_auto] md:items-start md:gap-8"
						>
							<span className="font-mono text-sm font-bold text-twin-accent-darker">
								{String(index + 1).padStart(2, "0")}
							</span>
							<div className="flex items-center gap-4">
								<span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-twin-accent-subtle text-twin-primary-main">
									<IconBox aria-hidden="true" icon={service.icon} className="size-5" />
								</span>
								<h2 className="text-balance text-2xl leading-tight font-bold text-twin-primary-main">
									{service.title}
								</h2>
							</div>
							...
							<Link
								...
								className="inline-flex min-h-12 items-center justify-center rounded-full border
									...
									no-underline transition-[background-color,color,transform] duration-160
			...
			<section className="bg-twin-light-1 px-5 py-24 md:px-10 md:py-28 lg:px-[8%]">
				<div className="mx-auto grid w-full max-w-350 gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
					<div>
						...
						<h2 className="mt-4 max-w-230 text-balance text-4xl leading-tight font-bold tracking-tight text-twin-primary-main md:text-5xl">
